...
WHEEL_MOMENT_OF_INERTIA = 6000*SIZE*SIZE
...
# Dynamics
BRAKE_FORCE = 2  # radians per second
FORWARD_FORCE = (0, 0.8)
SIDE_FORCE = (0.5, 0)
...
class Car:
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        for wx, wy in WHEELPOS:
            ...
            w = self.world.CreateDynamicBody(
                ...
                angle=init_angle,
                fixtures=fixtureDef(
                    shape=polygonShape(vertices=[(x*front_k*SIZE,y*front_k*SIZE) for x, y in self.wheel_poly]),
                    density=0.1,
                    categoryBits=0x0020,
                    maskBits=0x001,
                    restitution=0.0)
    ...
    def get_meta(self):
        ...
        meta_data = {}
        meta_data["size"] = SIZE
        meta_data["engine_power"] = ENGINE_POWER
        meta_data["wheel_moment_of_inertia"] = WHEEL_MOMENT_OF_INERTIA
        meta_data["friction_limit"] = FRICTION_LIMIT
        meta_data["wheel_r"] = WHEEL_R
        meta_data["wheel_w"] = WHEEL_W
        meta_data["wheel_pos"] = WHEELPOS
        meta_data["hull_poly1"] = HULL_POLY1
        meta_data["hull_poly2"] = HULL_POLY2
        meta_data["hull_poly3"] = HULL_POLY3
        meta_data["hull_poly4"] = HULL_POLY4
        meta_data["wheel_color"] = WHEEL_COLOR
        meta_data["wheel_white"] = WHEEL_WHITE
        meta_data["mud_color"] = MUD_COLOR
        meta_data["forward_force"] = FORWARD_FORCE
        meta_data["brake_force"] = BRAKE_FORCE
        meta_data["side_force"] = SIDE_FORCE
    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            # Force
            forw = w.GetWorldVector(FORWARD_FORCE)
            side = w.GetWorldVector(SIDE_FORCE)
            v = w.linearVelocity
